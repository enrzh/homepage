...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({ config }) => {
  ...
  const getIcon = () => {
    ...
    if (condition === 'Clear') return isDay ? <Sun className={`${size} text-yellow-300`} /> : <Moon className={`${size} text-blue-200`} />;
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-white p-4 relative">
      ...
      <div className="flex items-center gap-4 md:gap-6">
        {getIcon()}
        <div>
          <div className="text-4xl md:text-5xl font-black tracking-tighter">{Math.round(weather.temperature)}°</div>
          <div className="text-xs md:text-sm font-bold text-white/40 flex items-center gap-1 uppercase tracking-wider">
             {weather.condition}
          </div>
        </div>
      </div>
